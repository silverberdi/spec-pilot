...
PostgreSQL persistence baseline for SpecPilot using Prisma under `apps/api`, limited to a non-domain operational metadata probe model and deterministic migrations.
...
The baseline Prisma schema MUST define exactly one non-domain operational metadata model (for example `app_metadata` with a primary key, value, and timestamps) used to prove migration application and a typed client round trip. The schema MUST NOT introduce product domain models or tables for projects, reviews, findings, budgets, prompts, usage, authentication, or users.

#### Scenario: Probe model exists without domain tables
- **WHEN** the Prisma schema is inspected
- **THEN** exactly one non-domain operational metadata model is defined and no product domain models are present
