...
The web console MUST present SpecPilot-branded baseline shell content with Spanish as the default operator-facing locale and MUST remain i18n-ready through a minimal translation or locale boundary. This slice MUST NOT implement complete product internationalization, accessibility polish, light/dark/system theme switching, or product domain screens.

#### Scenario: Spanish baseline shell renders
- **WHEN** the web console shell loads successfully
- **THEN** a SpecPilot-branded baseline shell is shown with Spanish default operator-facing copy and without product domain dashboards or theme-switcher product features

#### Scenario: Complete i18n and accessibility are deferred
- **WHEN** the web console baseline scope is inspected
- **THEN** full product i18n coverage and accessibility polish belonging to later slices are not required or claimed as delivered by this baseline
