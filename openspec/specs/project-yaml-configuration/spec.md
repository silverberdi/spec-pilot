...
Parse, schema-validate, version, and persist immutable ProjectConfigurationVersion snapshots from `.specpilot/project.yaml`, link the active version on Project, fail closed on invalid configuration, remain read-only toward target repositories, and expose operator-visible refresh/get API outcomes.
