# `us-w04-s02-creation-prompt-generation-001` — Deliver core behavior

## Parent

- Wave: `w04`
- Slice: `w04-s02-creation-prompt-generation`
- Expected OpenSpec change: `chg-w04-s02-creation-prompt-generation`
- Implementer: `cursor`

## Story

As the SpecPilot operator, I need to implement the core behavior of generate executor-specific complete creation prompt. so that the product advances with bounded, verifiable behavior.

## Acceptance criteria

1. The slice delivers the explicitly declared capability: Generate executor-specific complete creation prompt.
