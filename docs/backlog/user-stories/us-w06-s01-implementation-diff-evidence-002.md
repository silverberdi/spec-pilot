# `us-w06-s01-implementation-diff-evidence-002` — Provide validation and evidence
...
As the SpecPilot operator, I need this slice to resolve base, diff, files, task status, and reported checks. so that the product advances with verifiable, bounded behavior.
