# `us-w09-s03-local-data-migration-001` — Deliver core behavior

## Parent

- Wave: `w09`
- Slice: `w09-s03-local-data-migration`
- Expected OpenSpec change: `chg-w09-s03-local-data-migration`
- Implementer: `cursor`

## Story

As the SpecPilot operator, I need to implement the core behavior of migrate unauthenticated local data safely to first owner. so that the product advances with bounded, verifiable behavior.

## Acceptance criteria

1. The slice delivers the explicitly declared capability: Migrate unauthenticated local data safely to first owner.
