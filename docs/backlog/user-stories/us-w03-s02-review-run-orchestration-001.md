# `us-w03-s02-review-run-orchestration-001` — Deliver core behavior

## Parent

- Wave: `w03`
- Slice: `w03-s02-review-run-orchestration`
- Expected OpenSpec change: `chg-w03-s02-review-run-orchestration`
- Implementer: `cursor`

## Story

As the SpecPilot operator, I need to implement the core behavior of persist and execute review run state machine. so that the product advances with bounded, verifiable behavior.

## Acceptance criteria

1. The slice delivers the explicitly declared capability: Persist and execute review run state machine.
