# `us-w06-s01-implementation-diff-evidence-001` — Deliver core behavior

## Parent

- Wave: `w06`
- Slice: `w06-s01-implementation-diff-evidence`
- Expected OpenSpec change: `chg-w06-s01-implementation-diff-evidence`
- Implementer: `cursor`

## Story

As the SpecPilot operator, I need to implement the core behavior of resolve base, diff, files, task status, and reported checks. so that the product advances with bounded, verifiable behavior.

## Acceptance criteria

1. The slice delivers the explicitly declared capability: Resolve base, diff, files, task status, and reported checks.
