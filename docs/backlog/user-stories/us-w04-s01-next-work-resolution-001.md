# `us-w04-s01-next-work-resolution-001` — Deliver core behavior

## Parent

- Wave: `w04`
- Slice: `w04-s01-next-work-resolution`
- Expected OpenSpec change: `chg-w04-s01-next-work-resolution`
- Implementer: `cursor`

## Story

As the SpecPilot operator, I need to implement the core behavior of resolve next eligible work generically and via wave/slice configuration. so that the product advances with bounded, verifiable behavior.

## Acceptance criteria

1. The slice delivers the explicitly declared capability: Resolve next eligible work generically and via wave/slice configuration.
