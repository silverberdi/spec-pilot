# `us-w07-s02-sync-readiness-decision-001` — Deliver core behavior

## Parent

- Wave: `w07`
- Slice: `w07-s02-sync-readiness-decision`
- Expected OpenSpec change: `chg-w07-s02-sync-readiness-decision`
- Implementer: `cursor`

## Story

As the SpecPilot operator, I need to implement the core behavior of require exact PASS and all closure gates. so that the product advances with bounded, verifiable behavior.

## Acceptance criteria

1. The slice delivers the explicitly declared capability: Require exact PASS and all closure gates.
